...
export const DashBoardBook = () => {
  ...
  const handleSubmit = async () => {
    ...
    // On transforme les champs vides en null pour Prisma
  ...
  const renderBookCard = (book: Book) => (
    <div key={book.id} className={`bg-white border ${editingId === book.id ? 'border-slate-900 ring-1 ring-slate-900' : ''} rounded-2xl p-4 flex items-center justify-between group transition-all shadow-sm`}>
      <div className="flex items-center gap-4 truncate">
        <img src={book.img ?? ''} className="w-10 h-14 object-cover rounded-md shadow-sm bg-slate-100" alt="" />
        <div className="truncate">
          <p className="font-bold text-slate-800 text-sm truncate">{book.title}</p>
          <p className="text-sky-500 text-[10px] uppercase font-bold">{book.titleCategory || "Unique"}</p>
        </div>
      </div>
      <div className="flex gap-2">
        <button onClick={() => handleDelete(book.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-full transition-all">
          <Trash2 size={16} />
        </button>
        <button onClick={() => handleEditClick(book)} className="p-2 text-sky-400 hover:bg-sky-50 rounded-full transition-all">
          <Pencil size={16} />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="w-full flex flex-col items-center min-h-screen pb-10">
      ...
      <div className="w-full max-w-7xl flex flex-col items-start mb-5 px-4">
      ...
      <div className="w-full max-w-7xl mx-auto bg-white/80 backdrop-blur-md rounded-[40px] p-8 md:p-12 shadow-2xl flex flex-col lg:flex-row gap-10">
        ...
        <div className="w-full lg:w-2/3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className='flex flex-col gap-2'>
              ...
              <select name="category" value={formData.category} onChange={handleChange} className="border border-sky-700 bg-white rounded-full p-2 px-4 outline-none">
            ...
            <div className="md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4  border-sky-100 ">
              ...
              <div className='flex flex-col gap-2'>
                ...
                <label className="font-semibold text-slate-700 text-sm" >Grand Format</label>
                <input name="grandFormat" value={formData.grandFormat} onChange={handleChange} type="text" placeholder="15/05/2014" className="border border-sky-200 rounded-full p-2 px-4 outline-none focus:border-sky-700" />
              </div>
              <div className='flex flex-col gap-2'>
                <label className="font-semibold text-slate-700 text-sm ">Poche</label>
                <input name="poche" value={formData.poche} onChange={handleChange} type="text" placeholder="16/02/2017" className="border border-sky-200 rounded-full p-2 px-4 outline-none focus:border-sky-700" />
              </div>
              <div className='flex flex-col gap-2'>
                <label className="font-semibold text-slate-700 text-sm ">Collector</label>
                <input name="collector" value={formData.collector} onChange={handleChange} type="text" placeholder="04/04/2024" className="border border-sky-200 rounded-full p-2 px-4 outline-none focus:border-sky-700" />
              </div>
              <div className='flex flex-col gap-2'>
                <label className="font-semibold text-slate-700 text-sm ">Graphique</label>
                <input name="collector" value={formData.graph} onChange={handleChange} type="text" placeholder="04/04/2024" className="border border-sky-200 rounded-full p-2 px-4 outline-none focus:border-sky-700" />
              </div>
            </div>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <input name="imgCategory" value={formData.imgCategory} onChange={handleChange} type="text" className="border border-sky-700 bg-white rounded-full p-2 px-4 outline-none" />
            </div>

            <div className='flex flex-col gap-2'>
              <label className="font-semibold text-slate-900 ">Image Couverture (URL)</label>
              <input name="img" value={formData.img} onChange={handleChange} type="text" className="border border-sky-700 bg-white rounded-full p-2 px-4 outline-none" />
            </div>
            ...
            <div className='md:col-span-2 flex flex-col gap-2'>
              ...
              <input name="epubURL" value={formData.epubURL} onChange={handleChange} type="text" placeholder="https://..." className="border border-sky-700 bg-white rounded-full p-2 px-4 outline-none" />
            </div>
          ...
          <button onClick={handleSubmit} className={`mt-8 ${editingId ? 'bg-slate-900' : 'bg-sky-600'} text-white px-10 py-3 rounded-full font-bold hover:opacity-90 shadow-lg flex items-center gap-2 transition-all`}>
 ...
 {/* liste livre */}
<div className="w-full lg:w-1/3 border-l border-sky-100 lg:pl-10">
